...
def main():
    ...
    print(f"\n[04b_coldstart] After VarianceThreshold: {n_surviving} features")
    ...
    print(f"[04b_coldstart] PCA → {n_components} components, "
    ...
    kmeans.fit(X_pca)
    print(f"[04b_coldstart] KMeans fitted with {n_clusters} clusters")
    ...
    print(f"\n[04b_coldstart] Cluster label mapping (cold → full model):")
    print(cluster_map.to_string(index=False))
    print(f"\n[04b_coldstart] User-level agreement with full model: {agreement:.1%}")
    ...
    print(f"\n[04b_coldstart] Saved all artifacts to {CACHE_DIR}")
    print(f"[04b_coldstart] Ready for step_10 cold-start prediction.")
