...
def build_persona_library_from_kb() -> dict:
    """Returns {cluster_id: persona_dict} built from step_08/09 KB artifacts."""
...
def safe_top_from_prefix(row: pd.Series, prefix: str, top_n: int = 3):
    ...
    return [col.replace(prefix, "") for col in values.index]
...
def describe_popularity(row: pd.Series):
    ...
    if not pop_cols:
        return "unknown"
    return row[pop_cols].sort_values(ascending=False).index[0]
...
def main():
    ...
    if not CLUSTER_INTERPRETATION_IN.exists() or not CLUSTER_MOVIE_KB_IN.exists():
        raise FileNotFoundError(
            "[07_dashboard_personas] KB artifacts missing. "
            "Run step_08_cluster_profile_kb and step_09_movie_kb first."
        )

    features = pd.read_parquet(FEATURE_MATRIX_IN)
    cluster_assignments = pd.read_parquet(CLUSTER_ASSIGNMENTS_IN)

    df = features.merge(cluster_assignments[["user", "cluster"]], on="user", how="left")

    numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
    numeric_feature_cols = [c for c in numeric_cols if c != "cluster"]
